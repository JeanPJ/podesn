...
dc = [1e-1,5e-2,1e-2,5e-3,1e-3,1e-4,1e-5]
ec = 5e-3


size_list = np.empty(len(dc))
r_list = np.empty([len(dc),n_out])
for k,dc_num in enumerate(dc):
    red_esn = GalerkinESN(cur_esn,states,u_data_normal[:red_size],ec,dc_num)
    size_list[k] = red_esn.m
    y_esn = np.empty([simtime,n_out])
    y_gar = np.empty_like(y_esn)
    y_deim = np.empty_like(y_esn)
    for t1 in range(simtime):
        y_gar[t1] = red_esn.update(u_data_normal[t1],"deim").flatten()
...
io.savemat('deim_project_result.mat',save_dict)

...
load_dict = io.loadmat('pod_project_result.mat')

size_list = load_dict['size_list'].flatten()

r_list = load_dict['r_list']

plt.plot(size_list/1400*100,r_list,'o')
...
plt.semilogy(size_list*100/N,ec)
plt.ylabel("Energy Cutoff")
plt.xlabel("States proportion (%)")
plt.grid(True)
plt.show()
